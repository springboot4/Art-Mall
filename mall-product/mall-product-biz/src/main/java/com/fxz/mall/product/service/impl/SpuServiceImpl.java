...
public class SpuServiceImpl extends ServiceImpl<SpuMapper, Spu> implements SpuService {
    ...
    public Boolean delete(Long id) {
        ...
        List<Long> skuIds = skuService.list(Wrappers.<Sku>lambdaQuery().eq(Sku::getSpuId, id).select(Sku::getId)).stream().map(Sku::getId).collect(Collectors.toList());
        if (CollectionUtils.isNotEmpty(skuIds)) {
            //删除sku相关属性
            skuAttributeValueService.remove(Wrappers.<SkuAttributeValue>lambdaQuery().eq(SkuAttributeValue::getSpuId, id).in(SkuAttributeValue::getSkuId, skuIds));
            //删除sku
