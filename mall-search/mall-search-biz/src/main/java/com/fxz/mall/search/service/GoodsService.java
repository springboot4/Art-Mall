...
public class GoodsService {
     ...
     * 更新es索引中促销活动的信息
     *
     * @param afterData 促销信息
     * @param key       es促销key
     * @param goodsId   商品id
     * @param skuIds    x需要更新的商品id
     */
    @SneakyThrows
    public void updateEsGoodsIndexPromotions(EsPromotionGoods afterData, String key, Long goodsId, List<Long> skuIds) {
        // 根据spuid获取es中商品索引信息
        ...
        // 获取到要更新的sku的信息
        List<SkuDTO> skuDTOS = skuList.stream().filter(sku -> skuIds.contains(sku.getId()))
                .collect(Collectors.toList());
        ...
        skuDTOS.forEach(skuDTO -> {
            ...
            // 促销活动map添加促销信息
            promotionMapJson.put(key, afterData);
        ...
        elasticsearchClient.update(u -> u.index("product").id(goodsDTO.getId().toString()).doc(goodsDTO),
                EsGoodsDTO.class);
    }
     ...
     * @param key       es促销key
     */
    public void updateEsGoodsIndexAll(EsPromotionGoods afterData, String key) {
        ThreadUtil.execAsync(() -> {
            for (int start = 0; ; start += 1000) {
                int finalStart = start;
                SearchResponse<EsGoodsDTO> search = null;
                try {
                    search = elasticsearchClient.search(
                            s -> s.index("product").query(q -> q.matchAll(m -> m)).from(finalStart).size(1000),
                            EsGoodsDTO.class);
                } catch (IOException e) {
                ...
                hits.forEach(h -> {
                    EsGoodsDTO source = h.source();
                    updateEsGoodsIndexPromotions(afterData, key, source.getId(),
